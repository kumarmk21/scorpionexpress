...
export function Warehousing_Router(request) {
   ...
   if (data) {
       ...
       // Render item page 
       return ok("Warehousing-page", data, seoData); 
   }
...
export function Warehousing_SiteMap(sitemapRequest) {
   ...
   const siteMapEntries = Object.keys(peopleData).map((name)=>{
                               ...
                               entry.pageName = "Warehousing-page";		// The name of the page in the Wix editor to render
                               entry.url = "/Warehousing/" + name ;			// Relative URL of the page
